...
def createDB():
    ...
    db.execute(
        'create table ServerNormalization (time_stamp text, temperature double, humidity double, pressure double, pitch double, roll double, yaw double, magnitude_x double, magnitude_y double, magnitude_z double, acceleration_x double, acceleration_y double, acceleration_z double, gyroscope_x double, gyroscope_y double, gyroscope_z double)')
    db.execute(
        'create table BalloonTicket (time_stamp text, balloon_id int, temperature double, humidity double, pressure double, pitch double, roll double, yaw double, magnitude_x double, magnitude_y double, magnitude_z double, acceleration_x double, acceleration_y double, acceleration_z double, gyroscope_x double, gyroscope_y double, gyroscope_z double)')
    db.commit()
    return db
...
def updateDataBase(db, str, balloonID):
    ...
    ##gyro_z = msg[15]
    db.execute(
        'insert into BalloonTicket (time_stamp, balloon_id, temperature, humidity, pressure, pitch, roll, yaw, magnitude_x, magnitude_y, magnitude_z, acceleration_x, acceleration_y, acceleration_z, gyroscope_x, gyroscope_y, gyroscope_z)  '
        'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', (
        msg[0], balloonID, msg[1], msg[2], msg[3], msg[4], msg[5], msg[6], msg[7], msg[8], msg[9], msg[10], msg[11],
...
def queues():  # This is a queue. We use it to queue things. very nice
    while True:
        if queueHandshake.full():
