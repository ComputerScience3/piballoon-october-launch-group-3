...
__credits__ = ['Ayush Agrawal', 'Andrew Bryan', 'Justin Carter', 'Christopher DeLaGarza', 'Deep Desai', 'Zachray Gray',
               'Joshua Harlan', 'Ryan Jacobs', 'Ryan King', 'Iori Koh', 'Sarath Muddana', 'Ryan Noeske',
               'Brett Phillips', 'Devin Popock',
               'Alyssa Rios', 'Ammar Sheikh', 'Cosme Tejeda', 'Ryan Vanek', 'Kyle Coffey', 'Zitao Fang', 'Chris Jung',
               'Taaha Kamal',
               'Maximilian Patrick', 'Weston Reed', 'Tanvir Towhid', 'Vance Vaughn', 'Abhi Velaga', 'Jacob Waller',
               'Colin Zhong', 'Taylor Hudson']
...
import socket #network communications stuffs
import sys #system commands / functions
from time import gmtime, strftime, sleep #manipulate the 4th dimension
from threading import Thread
import picamera
import csv #file reading/writing
import os #system specific stuff
from sense_hat import SenseHat

## Define sense from SenseHat() to use to collect data
sense = SenseHat()
sense.set_imu_config(False, True, False) #Enables and disables the gyroscope, accelerometer and/or magnetometer
## Define variables
directory = '/'
videoLength = 60 #seconds
camera = None
filename = strftime('%Y-%m-%d--%H:%M:%S') + '.csv' #formatting stuff
## Fields of Data
fieldnames = ['time', 'temperature', 'humidity', 'pressure', 'pitch', 'roll', 'yaw', 'mag_x', 'mag_y', 'mag_z', 'acc_x',
              'acc_y', 'acc_z', 'gyro_x', 'gyro_y', 'gyro_z'] #names for data
logfile = None
logwriter = None
...
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #creates connection to collect datas idk lol
...
#turn red then green if everything is ok
if (SenseHatAttached):
    FullScreen(255, 0, 0)
if (CameraAttached):
...
class myThread(Thread): #very nice class name
    def __init__(self, counter): #init thread and take video/pic/report data every
...
def REPORT():
    ...
    # send data to server
    finalData = strftime('%Y-%m-%d %H:%M:%S') + ',' + str(temp) + ',' + str(humidity) + ',' + str(
        pressure) + ',' + '{pitch}'.format(**orientation) + ',' + '{roll}'.format(**orientation) + ',' + '{yaw}'.format(
        **orientation) + ',' + '{x}'.format(**mag) + ',' + '{y}'.format(**mag) + ',' + '{z}'.format(
        **mag) + ',' + '{x}'.format(**acc) + ',' + '{y}'.format(**acc) + ',' + '{z}'.format(**acc) + ',' + '{x}'.format(
        **gyro) + ',' + '{y}'.format(**gyro) + ',' + '{z}'.format(**gyro) + ','
